...
export default function BillingPage() {
  ...
  async function startCheckout(plan: PlanId) {
    ...
    try {
      ...
      const billingResult = await apiFetch<BillingMe>("/billing/me");
      const usageResult = await apiFetch<PlanUsage>("/billing/usage");

      setBilling(billingResult);
      setUsage(usageResult);
      setMessage(
        `Checkout mock criado para ${checkout.plan}. URL: ${checkout.checkoutUrl}`,
      );
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        title="Plano"
        description="Escolha o plano do Promohub para sua operacao."
      />
      ...
      {billing ? (
        <section className="mb-6 rounded-lg border border-slate-200 bg-white p-5">
          ...
          <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3">
            ...
            <SummaryItem
              label="Status"
              value={readSubscriptionStatus(billing.subscriptionStatus)}
            />
            <SummaryItem
              label="Ultimo checkout"
              value={billing.subscription?.plan ?? "Nenhum"}
            />
          </dl>
        </section>
      ) : null}
...
function readButtonLabel(
  ...
  return loading ? "Criando checkout..." : "Iniciar checkout mock";
}
